    ...
    def compare(self):
        try:
            if self.comparator in ["equal", "equals", "相等", "字符相等"]:  # 等于
                ...
                assertpy.assert_that(self.actual_result).is_greater_than(LMAssert.str2num(self.expected_result))
                ...
                assertpy.assert_that(self.actual_result).is_greater_than_or_equal_to(LMAssert.str2num(self.expected_result))
            elif self.comparator in ["isLessThan", " 小于"]:  # 小于
                assFailMsg = '断言 实际值({}) 小于 预期值({})，断言失败：'.format(self.actual_result, self.expected_result)
                assertpy.assert_that(self.actual_result).is_less_than(LMAssert.str2num(self.expected_result))
                ...
                assertpy.assert_that(self.actual_result).is_less_than_or_equal_to(LMAssert.str2num(self.expected_result))
                ...
                assertpy.assert_that(self.actual_result).is_between(*LMAssert.str2list(self.expected_result))
            elif self.comparator in ["isCloseTo", " 接近于"]:  # 接近于
                assFailMsg = '断言 实际值({}) 接近于 预期值({})，断言失败：'.format(self.actual_result, self.expected_result)
                assertpy.assert_that(self.actual_result).is_close_to(*LMAssert.str2list(self.expected_result))
